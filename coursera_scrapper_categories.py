...
# Use https://www.kenst.com/2015/03/installing-chromedriver-on-mac-osx/ to install the driver on MacOs
...
def process_course_info(card_content: WebElement, title_class_name='card-title',
                        ...
                        level_class_name='difficulty') -> CourseInfo:
    try:
...
def process_page(chrome_driver, coursera_subject,FL_category, duration,timeout=10, result_class_name='ais-InfiniteHits-list', item_class_name='card-content'):
    ...
    try:
        ...
        try:
            chrome_driver.find_element(by=By.CLASS_NAME, value='default-results-icon').text
            default = True
        except NoSuchElementException:
            default = False
...
def start_scraping():
    ...
    writer.writerow(('index','title', 'partner', 'product_type', 'enrollment', 'coursera_subject','FL_category','duration','rating','level'))
